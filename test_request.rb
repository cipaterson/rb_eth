...
OptionParser.new do |opts|
  ...
  opts.on('-m', '--methods METHOD1,METHOD2', Array, 'Method name(s) to test, e.g. eth_getBalance (all methods by default).') do |methods|
    options[:methods] = methods
  end

  opts.on('-n', '--networks NETWORK1,NETWORK2', Array, 'Network subdomain to test (all test networks by default).') do |networks|
    options[:networks] = networks
  end

  opts.on('-k', '--api_key INFURA_API_KEY', String, 'Infura API Key to use (INFURA_API_KEY env var used by default).') do |api_key|
    options[:api_key] = api_key
  end
end.parse!

infura_api_key = options[:api_key] || ENV['INFURA_API_KEY'] || ENV['ETH']
if infura_api_key.nil?
  $stderr.puts 'Error: Use --api_key to provide a valid Infura API key'
  puts parser.help
  exit(1)
end
...
theNetworks = options[:networks] || defaultNetworks

defaultMethods = [
    {'method': 'eth_accounts', 'params': []},
    {'method': 'eth_blockNumber', 'params': []},
    {'method': 'eth_call', 'params': [{"from": "0xb60e8dd61c5d32be8058bb8eb970870f07233155", "to": "0xd46e8dd67c5d32be8058bb8eb970870f07244567", "gas": "0x76c0",
...
if options[:methods]
  # copy {methonds, params} across from the defaultMethods list
  options[:methods].each do |m|
    aMethod = defaultMethods.select {|e| e[:method]==m}[0]
    if aMethod
      theMethods << aMethod
    end
  end
  ...
  defaultMethods.each do |m|
    if nil == problemMethods.index {|e| e==m[:method]}
      theMethods << m
    end
  end
end
...
puts("method, " + theNetworks.join(', '))

theMethods.each do |m|
  m.store(:jsonrpc, '2.0')
  m.store(:id, '1')
  summary = m[:method]
  theNetworks.each do |n|
    uri = URI("https://#{n}.infura.io/v3/#{infura_api_key}")
    begin
      response = Net::HTTP.post(uri, JSON.generate(m))
    rescue SocketError
      puts "Error: Can't connect: #{uri}"
      exit(1)
    end
    if (response.code == "401")
      puts 'Error: Status code 401, use --api_key to provide a valid Infura API key'
      exit(1)
    end
    ...
    if response.code != "200" && response.code != "400"
      puts "Error: #{n}, #{m[:method]}, Status code: #{response.code}, URL: #{uri}"
      exit(1)
    end
    jResponse = JSON.load(response.body)

    # Print more detailed info if methods are specified on command line with '-m'
    if methods_on_cli
      if !jResponse.has_key?("error")
        puts "#{n}: #{jResponse["result"]}"
      else
        puts "#{n}: #{jResponse["error"]}"
      end
    end
